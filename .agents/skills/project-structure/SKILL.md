...
|---|---|
| `page.tsx`, `layout.tsx`, `template.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx` | `features/`, `components/`, `constants/`, `types/`, `services/`, `utils/`, `hooks/`, `providers/`, `store/` |
...
Un archivo de ruta se mantiene delgado: compone una feature de `features/<feature>/` y le pasa los parámetros de la ruta. La UI de una página vive en su carpeta de feature, no al lado de la ruta.

## 2. Imports

- Entre carpetas, siempre por alias: `@/components/ui`, `@/features/<feature>/<Feature>`, `@/constants`, `@/types/<archivo>.types`.
...
- `supabase/functions/` es código Deno, fuera del programa TypeScript de Next.js (está excluido en `tsconfig.json`). No importar nada de ahí desde `app/`, `features/` o `components/`, ni al revés.
...
1. **Relativa al proyecto**, escrita desde la raíz: `features/<feature>/`, `constants/index.ts`, `.agents/skills/project-structure/SKILL.md`.
...
- [ ] Las rutas son delgadas y delegan la UI a `features/<feature>/`
